import OffreVenteServies from "../services/offre.service.js"


class OffreControle{
    getAll = async(req, res) => {
        try{
            const data = await OffreVenteServies.selectProdiutAll();
    ...
    getById = async(res, req) => {
        ...
        const data = OffreVenteServies.getProduitById(id);
        if(!data) return res.status(404).json({message : "data not fond"});
        res.status(201).json(data)
    };
    ...
    delete = async(req, res) =>{
        try{
            const id = req.params.id;
            const deleResult = await OffreVenteServies.deleteProduitById(id);
    ...
    update = async(req, res) => {
        try{
            const id = req.params.id;
            const userData = await OffreVenteServies.updateProduitById(id, req.body);
...
export default new OffreControle();