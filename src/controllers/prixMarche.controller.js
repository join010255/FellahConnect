import PrixMarcheServes from "../services/prixMarche.service.js"


class PrixMarcheControle{
    getAll = async(req, res) => {
        try{
            const data = await PrixMarcheServes.selectProdiutAll();
    ...
    getById = async(res, req) => {
        ...
        const data = PrixMarcheServes.getProduitById(id);
        if(!data) return res.status(404).json({message : "data not fond"});
        res.status(201).json(data)
    };
    ...
    delete = async(req, res) =>{
        try{
            const id = req.params.id;
            const deleResult = await PrixMarcheServes.deleteProduitById(id);
    ...
    update = async(req, res) => {
        try{
            const id = req.params.id;
            const userData = await PrixMarcheServes.updateProduitById(id, req.body);
